...
// MODIFIED
// -----------------------------------------------------------------------------
const IntVect MappedPiecewiseLinearFillPatch::s_stencil_radius = IntVect::Unit;
...
// MODIFIED
// -----------------------------------------------------------------------------
MappedPiecewiseLinearFillPatch::MappedPiecewiseLinearFillPatch ()
: m_is_defined(false)
{;}
...
// MODIFIED
// -----------------------------------------------------------------------------
MappedPiecewiseLinearFillPatch::MappedPiecewiseLinearFillPatch (const DisjointBoxLayout& a_fine_domain,
...
// MODIFIED
// -----------------------------------------------------------------------------
void MappedPiecewiseLinearFillPatch::define (const DisjointBoxLayout& a_fine_domain,
                                             const DisjointBoxLayout& a_coarse_domain,
                                             int a_num_comps,
                                             const Box& a_crse_problem_domain,
                                             const IntVect& a_ref_ratio,
...
void MappedPiecewiseLinearFillPatch::define (const DisjointBoxLayout& a_fine_domain,
                                             ...
                                             bool a_pwconst_interp_only)
{
    CH_TIME("MappedPiecewiseLinearFillPatch::define");
...
// -----------------------------------------------------------------------------
// -----------------------------------------------------------------------------
bool MappedPiecewiseLinearFillPatch::isDefined () const
{
    return m_is_defined;
}
...
// -----------------------------------------------------------------------------
void MappedPiecewiseLinearFillPatch::fillInterp (LevelData<FArrayBox>& a_fine_data,
                                                 ...
                                                 int a_num_comp)
{
    CH_TIME("MappedPiecewiseLinearFillPatch::fillInterp");
...
// -----------------------------------------------------------------------------
void MappedPiecewiseLinearFillPatch::fillInterpPWConstSpace (LevelData<FArrayBox>& a_fine_data,
...
// Time interpolation of coarse level data, to coarsened fine level work array
// -----------------------------------------------------------------------------
void MappedPiecewiseLinearFillPatch::timeInterp (const LevelData<FArrayBox>& a_old_coarse_data,
                                                 const LevelData<FArrayBox>& a_new_coarse_data,
                                                 Real a_time_interp_coef,
                                                 int a_src_comp,
                                                 int a_dest_comp,
                                                 int a_num_comp)
{
    CH_TIME("MappedPiecewiseLinearFillPatch::timeInterp");
    Interval src_interval (a_src_comp,  a_src_comp  + a_num_comp - 1);
    Interval dest_interval(a_dest_comp, a_dest_comp + a_num_comp - 1);

    if ((a_old_coarse_data.boxLayout().size() == 0) && (a_new_coarse_data.boxLayout().size() == 0)) {
        MayDay::Error ( "MappedPiecewiseLinearFillPatch::fillInterp: no old coarse data and no new coarse data" );
...
// -----------------------------------------------------------------------------
void MappedPiecewiseLinearFillPatch::fillConstantInterp (LevelData<FArrayBox>& a_fine_data,
                                                         int a_src_comp,
                                                         int a_dest_comp,
                                                         int a_num_comp) const
{
    CH_TIME("MappedPiecewiseLinearFillPatch::fillConstantInterp");
...
// -----------------------------------------------------------------------------
void MappedPiecewiseLinearFillPatch::computeSlopes (int a_src_comp,
                                                    int a_num_comp)
{
    CH_TIME("MappedPiecewiseLinearFillPatch::computeSlopes");
...
// -----------------------------------------------------------------------------
// -----------------------------------------------------------------------------
void MappedPiecewiseLinearFillPatch::computeSimpleSlopesFab (FArrayBox&        a_slopeFab,
...
{
    CH_TIME("MappedPiecewiseLinearFillPatch::computeSimpleSlopes");
...
// -----------------------------------------------------------------------------
// -----------------------------------------------------------------------------
void  MappedPiecewiseLinearFillPatch::computeMultiDimSlopes(FArrayBox&       a_slopes0,
        FArrayBox&       a_slopes1,
        FArrayBox&       a_slopes2,
        const FArrayBox& a_dataFab,
        const int&       a_src_comp,
...
// MODIFIED
// -----------------------------------------------------------------------------
void MappedPiecewiseLinearFillPatch::incrementLinearInterp (LevelData<FArrayBox>& a_fine_data,
                                                            int a_src_comp,
                                                            int a_dest_comp,
                                                            int a_num_comp) const
{
    CH_TIME("MappedPiecewiseLinearFillPatch::incrementLinearInterp");
    for (int dir = 0; dir < SpaceDim; dir++) {
        DataIterator dit = a_fine_data.boxLayout().dataIterator();
        for (dit.begin(); dit.ok(); ++dit) {
...
// -----------------------------------------------------------------------------
// -----------------------------------------------------------------------------
void MappedPiecewiseLinearFillPatch::printIntVectSets () const
{
