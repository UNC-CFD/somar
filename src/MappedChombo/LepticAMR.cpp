...
int LepticAMR::s_step = 0;

//-----------------------------------------------------------------------
void LepticAMR::useSubcyclingInTime(bool a_useSubcycling)
{
...
//-----------------------------------------------------------------------
void LepticAMR::setDefaultValues()
{
    m_isDefined = false;
    m_isSetUp   = false;
...
//-----------------------------------------------------------------------
void LepticAMR::clearMemory()
{
    CH_TIME("LepticAMR::clearMemory");
...
//-----------------------------------------------------------------------
void LepticAMR::maxGridSize(IntVect a_max_grid_size)
{
    CH_TIME("LepticAMR::maxGridSize");
...
//-----------------------------------------------------------------------
void LepticAMR::maxBaseGridSize(IntVect a_max_base_grid_size)
{
...
// 0 = no splitting
// 1 = split
void LepticAMR::splitDirs(IntVect a_splitDirs)
{
...
// 0 = no splitting
// 1 = split
void LepticAMR::splitDirs(Vector<int> a_splitDirs)
{
...
//-----------------------------------------------------------------------
void LepticAMR::dtToleranceFactor(Real a_dt_tolerance_factor)
{
...
//-----------------------------------------------------------------------
void LepticAMR::gridBufferSize(int a_grid_buffer_size)
{
    CH_TIME("LepticAMR::gridBufferSize");
...
//-----------------------------------------------------------------------
IntVect LepticAMR::maxGridSize() const
{
    CH_assert(isDefined());
...
//-----------------------------------------------------------------------
IntVect LepticAMR::maxBaseGridSize() const
{
    CH_assert(isDefined());
...
//-----------------------------------------------------------------------
void LepticAMR::plotPrefix(const std::string& a_plotfile_prefix)
{
    CH_assert(isDefined());
...
//-----------------------------------------------------------------------
void LepticAMR::blockFactor(int a_blockFactor)
{
    CH_TIME("LepticAMR::blockFactor");
...
//-----------------------------------------------------------------------
void LepticAMR::fillRatio(Real a_fillRatio)
{
    CH_TIME("LepticAMR::fillRatio");
...
//-----------------------------------------------------------------------
void LepticAMR::checkpointPrefix(const std::string& a_checkpointfile_prefix)
{
    CH_assert(isDefined());
...
//-----------------------------------------------------------------------
void LepticAMR::regridIntervals(const Vector<int>& a_regridIntervals)
{
    CH_assert(isDefined());
...
//-----------------------------------------------------------------------
void LepticAMR::plotInterval(int a_plot_interval)
{
...
//-----------------------------------------------------------------------
void LepticAMR::plotPeriod(Real a_plot_period)
{
...
//-----------------------------------------------------------------------
void LepticAMR::checkpointInterval(int a_checkpoint_interval)
{
...
//-----------------------------------------------------------------------
void LepticAMR::define(int                                a_max_level,
...
void LepticAMR::define(int                                a_max_level,
                       ...
                       const MappedAMRLevelFactory* const a_amrLevelFact)
{
    CH_TIME("LepticAMR::define");

    clearMemory();

    // Function already called in LepticAMR() ... so make sure you set any values
    // _after_ the define() function. (ndk)
    setDefaultValues();

    const ProblemContext* ctx = ProblemContext::getInstance();
    splitDirs(ctx->splitDirs);

    m_isDefined = true;
    CH_assert(a_ref_ratios.size() > a_max_level);

    if (m_verbosity >= 3) {
        pout() << "LepticAMR::define(...)" << endl;
    }
...
//-----------------------------------------------------------------------
void LepticAMR::setupForFixedHierarchyRun(const Vector<Vector<Box> >& a_amr_grids,
                                          int                         a_proper_nest)
{
    CH_TIME("LepticAMR::setupForFixedHierarchyRun");
    ...
    if (m_verbosity >= 3) {
        pout() << "LepticAMR::setupForFixedHierarchyRun(...)" << endl;
    }
    ...
    if (m_finest_level > m_max_level) {
        cerr << "LepticAMR::setupForFixedHierarchy: too many levels in input grids" << endl;
        cerr << "max_level from define function = " << m_max_level << endl;;
        cerr << "finest level(top level of grids in setup) = = " << m_finest_level << endl;
        MayDay::Error("LepticAMR::setupForFixedHierarchy: define call and setup call inconsistent in class LepticAMR");
    }
...
//-----------------------------------------------------------------------
void LepticAMR::setupForNewAMRRun()
{
    CH_TIME("LepticAMR::setupForNewAMRRun");
    ...
    if (m_verbosity >= 3) {
        pout() << "LepticAMR::setupForNewAMRRun" << endl;
    }
...
void LepticAMR::setupForRestart(HDF5Handle& a_handle)
{
    CH_TIME("LepticAMR::setupForRestart");
    ...
    if (header.m_int.find("max_level") == header.m_int.end()) {
        MayDay::Error("LepticAMR::restart: checkpoint file does not contain max_level");
    }
    ...
    if (max_level_check != m_max_level) {
        pout() << "LepticAMR::restart: checkpoint file inconsistent with inputs to define " << endl;
        pout() << "max level input to define = " << m_max_level << endl;
        pout() << "max level in checkpoint = " << max_level_check << endl;
        MayDay::Warning("LepticAMR::restart: checkpoint file inconsistent with inputs to define ");
    }
    ...
    if (header.m_int.find("num_levels") == header.m_int.end()) {
        MayDay::Error("LepticAMR::restart: checkpoint file does not contain num_levels");
    }
    ...
    if (m_finest_level > m_max_level) {
        ...
        MayDay::Error("LepticAMR::restart: checkpoint file inconsistent with inputs to define ");
    }
    ...
    if (m_finest_level > m_max_level) {
        MayDay::Error("LepticAMR::restart: finest_level > max_level");
    }

    if (header.m_int.find("iteration") == header.m_int.end()) {
        MayDay::Error("LepticAMR::restart: checkpoint file does not contain iteration");
    }
    ...
    if (header.m_real.find("time") == header.m_real.end()) {
        MayDay::Error("LepticAMR::restart: checkpoint file does not contain time");
    }
    ...
    for (int level = 0; level < m_finest_level; ++level) {
        IntVect refratio_test = m_amrlevels[level]->refRatio();
...
//-----------------------------------------------------------------------
void LepticAMR::conclude() const
{
    CH_TIME("LepticAMR::conclude");

    CH_assert(isDefined());
    CH_assert(isSetUp());

    if (m_verbosity >= 3) {
        pout() << "LepticAMR::conclude" << endl;
    }
...
//-----------------------------------------------------------------------
// go baby go
void LepticAMR::run(Real a_max_time, int a_max_step)
{
    CH_TIME("LepticAMR::run");
...
#endif
    if (m_verbosity >= 3) {
        pout() << "LepticAMR::coarseTimeStep:" << endl;
...
void LepticAMR::assignDt()
{
    CH_TIME("LepticAMR::assignDt");

    CH_assert(isDefined());

    if (m_verbosity >= 3) {
        pout() << "LepticAMR::assignDt" << endl;
    }

    if (m_useSubcycling) {
        // MayDay::Error("LepticAMR::assignDt with subcycling needs to be fixed");
        ...
        if (m_fixedDt > 0) {
            ...
            for (int level = 1; level <= m_finest_level_old; ++level) {
                ...
                D_TERM(
                int eff_ref_factor = ref_factor[0];,
            ...
            for (int level = 0; level <= m_finest_level_old; ++level) {
                ...
                Real dt_base_equiv = m_dt_cur[level] * eff_ref_factor * m_maxDtGrow;
                m_dt_base = Min(m_dt_base, dt_base_equiv);
            }
        }
...
int LepticAMR::timeStep(int a_level, int a_stepsLeft, bool a_coarseTimeBoundary)
{
    CH_TIME("LepticAMR::timeStep");
    CH_assert(isDefined());
    CH_assert(isSetUp());
    if (m_verbosity >= 3) {
        pout() << "LepticAMR::timeStep(" << a_level << ")" << endl;
    }

    if (m_useSubcycling) {
        if (a_level < m_max_level) {
            CH_TIME("LepticAMR::timeStep::regrid");
            if (m_verbosity >= 4) {
        ...
        if ((!a_coarseTimeBoundary) && (m_fixedDt <= 0)) {
            CH_TIME("LepticAMR::timeStep::subcycle");
        ...
        {
            // advance this level
            CH_TIME("LepticAMR::timeStep::advance");
        ...
        {
            // get the new dt
            CH_TIME("LepticAMR::timeStep::newDt");
        ...
        if (a_level < m_finest_level) {
            CH_TIME("LepticAMR::timeStep::finerLevels");

            D_TERM(
            int stepsLeft = m_ref_ratios[a_level][0];,
        ...
        {
            CH_TIME("LepticAMR::timeStep::postTimeStep");
        ...
        if (a_level != 0) {
            MayDay::Error("LepticAMR::timestep is confused by being called for a_level != 0 with subcycling off");
        }
        if (a_level < m_max_level) {
            CH_TIME("LepticAMR::timeStep::regrid");
            if (m_verbosity >= 4) {
            ...
            if (regridThisLevel) {
                regrid(a_level);
                m_steps_since_regrid[a_level] = 1; // Changed from 0 to 1 by ES - this step counts!
            } else {
                m_steps_since_regrid[a_level]++;
            }
        }
...
//-----------------------------------------------------------------------
bool LepticAMR::needToRegrid(int a_level, int a_stepsLeft) const
{
    CH_TIME("LepticAMR::needToRegrid");
    ...
    if (m_verbosity >= 3) {
        pout() << "LepticAMR::needToRegrid(" << a_level << ")" << endl;
    }
...
void LepticAMR::regrid(int a_base_level)
{
    CH_TIME("LepticAMR::regrid");

    CH_assert(isDefined());
    CH_assert(isSetUp());

    if (m_verbosity >= 4) {
        pout() << "LepticAMR::regrid(" << a_base_level << ")" << endl;
    }

    if (m_verbosity >= 4) {
        pout() << "LepticAMR::regrid: base level = " << a_base_level << endl;
    }
    ...
    if (m_use_meshrefine) {
        for (int level = a_base_level; level <= top_level; ++level) {
            ...
            if (m_verbosity >= 4) {
                pout() << "LepticAMR::regrid: problem domain[" << level << "]: " << problem_domains[level] << endl;
            }

            if (m_verbosity >= 4) {
                pout() << "LepticAMR::regrid: old_grids[" << level << "]: " << endl;
                ...
                if (m_verbosity >= 5) {
                    pout() << "LepticAMR::regrid: tags[" << level << "]: " << tags[level]
                           << endl;
                }
            }
        }
...
//-----------------------------------------------------------------------
Vector<MappedAMRLevel*> LepticAMR::getAMRLevels()
{
    return m_amrlevels;
}
//-----------------------------------------------------------------------
...
//-----------------------------------------------------------------------
void LepticAMR::initialTime(Real a_initialTime)
{
    ...
    // propagate this time to all of the LepticAMR levels
    for (int lev = 0; lev < m_amrlevels.size(); lev++) {
...
//-----------------------------------------------------------------------
Real LepticAMR::getCurrentTime() const
{
    return m_cur_time;
}
//-----------------------------------------------------------------------
...
//-----------------------------------------------------------------------
void LepticAMR::makeBaseLevelMesh(Vector<Box>& a_grids) const
{
    CH_TIME("LepticAMR::makeBaseLevelMesh");
    ...
    if (m_max_base_grid_size == IntVect::Zero) {
        ...
        D_TERM(
        if (m_max_base_grid_size[0] != 0 && m_max_base_grid_size[0] < m_blockFactor) {
            MayDay::Abort("Base grid size must be greater than blocking factor");
        },
        if (m_max_base_grid_size[1] != 0 && m_max_base_grid_size[1] < m_blockFactor) {
            MayDay::Abort("Base grid size must be greater than blocking factor");
        },
        if (m_max_base_grid_size[2] != 0 && m_max_base_grid_size[2] < m_blockFactor) {
            MayDay::Abort("Base grid size must be greater than blocking factor");
        })
...
// BEGIN: This may need to be debugged.
        ...
        const IntVect Nx = fullDomain.size();
        const bool isPeriodic[CH_SPACEDIM] =
        {
            D_DECL(fullDomain.isPeriodic(0),
        ...
        IntVect blockMaxGridSize = m_splitDirs * m_max_base_grid_size / m_blockFactor;
        ...
        for (int dir = 0; dir < SpaceDim; ++dir) {
            ...
            if (m_splitDirs[dir] == 0) {
                ...
                while (num_div * blockMaxGridSize[dir] < blockDomSize) ++num_div;
                ...
                box_sizes[dir][num_div-1] = blockDomSize - (num_div-1)*base_size[dir];
            }
        }
...
void LepticAMR::initialGrid()
{
    CH_TIME("LepticAMR::initialGrid");

    CH_assert(m_isDefined);

    if (m_verbosity >= 3) {
        pout() << "LepticAMR::initialGrid" << endl;
    }
    ...
    if (m_use_meshrefine) {
        for (int top_level = 0;
                ...
                ++top_level) {
            if (m_verbosity >= 2) {
                pout() << "LepticAMR::initialGrid: top level = " << top_level << endl;
            }
            ...
            for (int level = 0; level <= top_level; ++level) {
                ...
                if (m_verbosity >= 3) {
                    pout() << "LepticAMR::initialGrid: problem domain["
                           << level << "]: " << problem_domains[level] << endl;
                }

                if (m_verbosity >= 4) {
                    pout() << "LepticAMR::initialGrid: old_grids[" << level << "]: " << endl;
                    ...
                    if (m_verbosity >= 5) {
                        pout() << "LepticAMR::initialGrid: tags[" << level << "]: "
                               << tags[level] << endl;
                    }
                }
            }
...
//-----------------------------------------------------------------------
void LepticAMR::writePlotFile() const
{
    CH_TIME("LepticAMR::writePlotFile");

    CH_assert(m_isDefined);

    if (m_verbosity >= 3) {
        pout() << "LepticAMR::writePlotFile" << endl;
    }
...
//-----------------------------------------------------------------------
void LepticAMR::writeCheckpointFile() const
{
    CH_TIME("LepticAMR::writeCheckpointFile");

    CH_assert(m_isDefined);

    if (m_verbosity >= 3) {
        pout() << "LepticAMR::writeCheckpointFile" << endl;
    }
...
//-----------------------------------------------------------------------
void LepticAMR::verbosity(int a_verbosity)
{
    m_verbosity = a_verbosity;

    if (m_verbosity >= 2) {
        pout() << " LepticAMR::verbosity  m_verbosity=" << m_verbosity << endl;
    }
}
//-----------------------------------------------------------------------

//-----------------------------------------------------------------------
int LepticAMR::verbosity() const
{
    return (m_verbosity);
}
//-----------------------------------------------------------------------
...
//-----------------------------------------------------------------------
void LepticAMR::maxDtGrow(Real a_dtGrowFactor)
{
...
//-----------------------------------------------------------------------
Real LepticAMR::maxDtGrow() const
{
    return m_maxDtGrow;
}
//-----------------------------------------------------------------------
...
//-----------------------------------------------------------------------
void LepticAMR::timeEps(Real a_timeEps)
{
...
//-----------------------------------------------------------------------
void LepticAMR::fixedDt(Real a_dt)
{
    m_fixedDt = a_dt;
}
//-----------------------------------------------------------------------
...
//-----------------------------------------------------------------------
bool LepticAMR::isDefined() const
{
    return m_isDefined;
}
//-----------------------------------------------------------------------
...
//-----------------------------------------------------------------------
#ifdef CH_USE_TIMER
Timer* LepticAMR::timer(Timer* a_timer )
{
...
//-----------------------------------------------------------------------
void
LepticAMR::schedule(RefCountedPtr<Scheduler> a_scheduler)
{
    CH_assert(!a_scheduler.isNull());
    m_scheduler = a_scheduler;

    // Set up the scheduler's periodic functions to interact with
    // this LepticAMR object.
    //  m_scheduler->setUp(*this);
    MayDay::Error("LepticAMR::schedule() needs to pass an AMR version of this to the scheduler");
}
//-----------------------------------------------------------------------
