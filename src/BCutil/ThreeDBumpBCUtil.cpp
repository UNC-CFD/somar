...
bool ThreeDBumpBCUtil::s_tidalParamsRead = false;
Real ThreeDBumpBCUtil::s_tidalOmega = 0.0;
Real ThreeDBumpBCUtil::s_tidalU0 = 0.0;
bool ThreeDBumpBCUtil::s_isNoSlip = false;
...
// -----------------------------------------------------------------------------
void ThreeDBumpBCUtil::bscalBCValues (Real*           a_pos,
                                          int*            a_dir,
...
// Default constructor
// -----------------------------------------------------------------------------
ThreeDBumpBCUtil::ThreeDBumpBCUtil ()
{
    ...
    s_isNoSlip = false; // set this to true for no-slip conditions on velocity
                        // at bottom boundary
}
...
// Factory
// -----------------------------------------------------------------------------
PhysBCUtil* ThreeDBumpBCUtil::newPhysBCUtil () const
{
    PhysBCUtil* newBCPtr = new ThreeDBumpBCUtil();
    return newBCPtr;
}
...
// -----------------------------------------------------------------------------
void ThreeDBumpBCUtil::computeBoundaryDt (Real&                a_dt,
                                              const Real           a_cfl,
...
// -----------------------------------------------------------------------------
void ThreeDBumpBCUtil::setScalarIC (FArrayBox&           a_scalarFAB,
...
// -----------------------------------------------------------------------------
void ThreeDBumpBCUtil::setBackgroundScalar (FArrayBox&           a_scalarFAB,
                                                const int            a_scalarComp,
                                                const LevelGeometry& a_levGeo,
...
// -----------------------------------------------------------------------------
void ThreeDBumpBCUtil::fillVelSpongeLayerTarget (FArrayBox&           a_target,
                                                     const int            a_velComp,
                                                     ...
                                                     const Real           a_time)
{
    ...
    if (a_spongeDir == 0) {
        ...
        MayDay::Error("ThreeDBumpBCUtil::fillVelSpongeLayerTarget "
                      "can only set a sponge target when a_spongeDir = 0");
    }
}

void tidalvel(Real* a_pos,
	      int* a_dir,
	      Side::LoHiSide* a_side,
	      Real* a_value,
	      const RealVect& a_dx,
...
{ static int s_dir;
  static Real s_oldTime=0.0;
  const ProblemContext* ctx = ProblemContext::getInstance();
  if(a_time != BOGUS_TIME) s_oldTime=a_time;
  s_dir= *a_dir;
  *a_value= (s_dir==0) ? ctx->tidalU0*sin(ctx->tidalOmega * s_oldTime)  : ctx->tidalU0*cos(ctx->tidalOmega * s_oldTime);
  // we copy a_time to a static variable because otherwise
  // we get an erratic behavior. Not really sure why.
  return;}
// -----------------------------------------------------------------------------
// basicVelFuncBC
// Sets physical BCs on velocities.
// -----------------------------------------------------------------------------
BCMethodHolder ThreeDBumpBCUtil::basicVelFuncBC (int a_veldir, bool a_isViscous) const
{
    ...
    if (a_veldir <SpaceDim-1 ) {
        //                  Freeslip
        // u: Diri inflow |==========| Diri outflow=inflow
        //                  Diri 0
	  ...
	  EllipticBCValueFunc myHolder;
	  myHolder = tidalvel;
        RefCountedPtr<BCGhostClass> fluxBCPtr(
            new EllipticDiriBCGhostClass(myHolder,
                                             BASISV(a_veldir),
                                             BASISV(a_veldir))
        );
...
// -----------------------------------------------------------------------------
Tuple<BCMethodHolder,SpaceDim> ThreeDBumpBCUtil::velSlopeBC (int a_velComp, bool a_isViscous) const
{
    BCMethodHolder holder;
...
// -----------------------------------------------------------------------------
BCMethodHolder ThreeDBumpBCUtil::diffusiveSourceFuncBC () const
{
    ...
    RefCountedPtr<BCGhostClass> neumBCPtr(
        new ThreeDBeamGeneratorNeumBCGhostClass(this)
    );
...
// -----------------------------------------------------------------------------
BCMethodHolder ThreeDBumpBCUtil::diffusiveSolveFuncBC () const
{
...
// -----------------------------------------------------------------------------
void ThreeDBeamGeneratorNeumBCGhostClass::operator() (FArrayBox&           a_state,
...
{
    CH_TIME("ThreeDBeamGeneratorNeumBCGhostClass::operator()");
